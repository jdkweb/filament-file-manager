...
namespace Jdkweb\FilamentFileManager\Form;
        ...
        $this->saveRelationshipsUsing(static function (Repeater $component, HasMedia $record): void {
            ...
            foreach ($setState as $item){
                $state = array_filter(array_map(function (TemporaryUploadedFile | string $file) use ($mediaComponent, $record, $component, $item, $counter) {
                    ...
                    $homeFolder = config('filament-file-manager.model.folder')::where('model_type', get_class($record))
                        ->where('model_id', null)
                    ...
                    if(!$homeFolder){
                        ...
                        if(filament('filament-file-manager')->allowUserAccess){
                            $data['user_id'] = auth()->user()->id;
                            $data['user_type'] =  get_class(auth()->user());
                        }
                        $homeFolder = config('filament-file-manager.model.folder')::create($data);
                    }

                    $collectionFolder = config('filament-file-manager.model.folder')::where('model_type', get_class($record))
                        ->where('model_id', null)
                    ...
                    if(!$collectionFolder){
                        ...
                        $collectionFolder = config('filament-file-manager.model.folder')::create($data);
                    }

                    $folder = config('filament-file-manager.model.folder')::where('collection', $component->name)
                    ...
                    if(!$folder){
                        ...
                        $folder = config('filament-file-manager.model.folder')::create($data);
                    }
